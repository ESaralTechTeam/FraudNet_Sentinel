...
from services.database import BeneficiaryDB, AlertDB, FraudNetworkDB
...
async def get_district_risk():
    ...
    try:
        ...
        # Calculate average risk per district
        result = []
        for district, data in districts.items():
            ...
            result.append({
                ...
                'risk_category': 'high' if avg_risk >= 0.6 else 'medium' if avg_risk >= 0.4 else 'low',
...
async def get_fraud_networks():
    ...
    try:
        ...
        # Get shared resource networks
        networks = patterns['shared_resources']
        ...
        return {
            ...
            'patterns': {
                ...
                'connected_clusters': len(patterns['connected_clusters']),
                'high_centrality': len(patterns['high_centrality_nodes'])
            }
        }
...
async def get_summary():
    ...
    try:
        ...
        high_risk = sum(1 for b in beneficiaries if b.get('risk_category') in ['high', 'critical'])
        duplicates = sum(1 for b in beneficiaries if b.get('is_duplicate'))
        flagged = sum(1 for b in beneficiaries if b.get('is_flagged'))
        
        total_amount = sum(b.get('amount', 0) for b in beneficiaries)
        high_risk_amount = sum(b.get('amount', 0) for b in beneficiaries if b.get('risk_category') in ['high', 'critical'])
